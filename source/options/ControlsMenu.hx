...
class ControlsMenu extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		for (i in 0...controlList.length)
		{
			...
			var isNote:Bool = name.toLowerCase().startsWith('note');
			...
			if (currentHeader != "UI_" && name.indexOf("UI_") == 0)
			{
				...
				if (addedHeaders.get("EXTRAS") == false) {
					currentHeader = "EXTRAS_";
					headers.add(new BoldText(0, y, "EXTRAS")).screenCenter(X);
					y += spacer;
					addedHeaders.set("EXTRAS", true);
				}
			}
