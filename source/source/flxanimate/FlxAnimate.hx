...
class FlxAnimate extends FlxSprite
{
	...
	public function loadAtlas(Path:String)
	{
		...
		#if desktop
		if (FileSystem.exists('$Path/Animation.json'))
			shouldReturn = false;
		#end
	...
	function atlasSetting(Path:String):AnimAtlas
	{
		...
		if (haxe.io.Path.extension(Path) == "zip")
		{
			...
			if (FileSystem.exists(Path))
				thing = Zip.readZip(File.getBytes(Path));
			else
			#end
				thing = Zip.readZip(Assets.getBytes(Path));
		...
		{
			if (FileSystem.exists('$Path/Animation.json'))
				jsontxt = haxe.Json.parse(File.getContent('$Path/Animation.json'));
			else
