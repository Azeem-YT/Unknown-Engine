...
class FlxAnimateFrames extends FlxAtlasFrames
{
    ...
    public static function getImage(id):Any
    {
        var bitmap:BitmapData = BitmapData.fromFile(Paths.modImages(id));
		var returnGraphic:FlxGraphic = FlxGraphic.fromBitmapData(bitmap, false, id);
    ...
    public static function fromTextureAtlas(Path:String):FlxAtlasFrames
    {
        ...
        if (zip != null || haxe.io.Path.extension(Path) == "zip")
        {
            ...
            var thing;
            #if desktop
            thing = (zip != null) ? zip :  Zip.unzip(Zip.readZip(File.getBytes(Path)));
            #else
            thing = (zip != null) ? zip :  Zip.unzip(Zip.readZip(Assets.getBytes(Path)));
            #end
        ...
        else
        {
            ...
            #end
            {
                var curJson:AnimateAtlas = haxe.Json.parse(StringTools.replace(#if desktop File.getContent #else Assets.getText #end('$Path/spritemap.json'), String.fromCharCode(0xFEFF), ""));
                var curSpritemap = getImage('$Path/${curJson.meta.image}');
            ...
            var i = 1;
            #if desktop
            while (FileSystem.exists('$Path/spritemap$i.json'))
            #else
            while (Assets.exists('$Path/spritemap$i.json'))
            #end
            {
                var curJson:AnimateAtlas = haxe.Json.parse(StringTools.replace(#if desktop File.getContent #else Assets.getText #end('$Path/spritemap$i.json'), String.fromCharCode(0xFEFF), ""));
                var curSpritemap = getImage('$Path/${curJson.meta.image}');
                if (curSpritemap != null)
                {
                    var graphic = FlxG.bitmap.add(curSpritemap);
	...
	{
        #if desktop
        if (Path is String && !FileSystem.exists(Path))
			return null;
        #else
		...
		var data:Access = new Access((Path is String) ? Xml.parse(File.getContent(Path)).firstElement() : Path.firstElement());
        if (Image == null)
        {
            if (Path is String)
            {
    ...
    public static function fromJson(Path:FlxJson, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        ...
        #end
            return null;
        var data:JsonNormal = (Path is String) ? haxe.Json.parse(#if desktop File.getContent #else Assets.getText #end(Path)) : Path;
        if (Image == null)
        {
            if (Path is String)
            {
    ...
    public static function fromStarling(Path:FlxPropertyList, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        ...
        #end
            return null;
        var data:Plist = (Path is String) ? PropertyList.parse(#if desktop File.getContent #else Assets.getText #end(Path)) : Path;
        if (Image == null)
        {
            if (Path is String)
            {
    ...
    public static function fromCocos2D(Path:String, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        ...
        #end
            return null;
        var data:Plist = PropertyList.parse(#if desktop File.getContent #else Assets.getText #end(Path));
    ...
    public static function fromEaselJS(Path:String, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        ...
        var separatedJS = #if desktop File.getContent #else Assets.getText #end(Path).split("\n");
        ...
        for (i in 0...names.length)
        {
            ...
            var name = names[i];
            var json = jsons[i];
            var bitmap = FlxG.bitmap.add(getImage((Image == null) ? '${imagePath.join("/")}/${json.images[0]}' : Image));
