	...
	public function addLibrary(name:String = '', packageNme:String = '') {
		if (hscript != null && (name != null && name != '')) {
			try {
				...
				hscript.interp.variables.set(name, packageStr + name);
			}
			catch(e:Dynamic) {
				trace(e);
			}
...
class GameCode
{
	...
	public function new() {
		...
		variables.set("game", PlayState.instance);//Psych Engine Support
		...
		variables.set('PlayState', PlayState);
		variables.set("ShaderFilter", ShaderFilter);
		#if desktop
