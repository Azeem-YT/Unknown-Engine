...
class ColorsMenu extends MusicBeatSubstate
{
	...
	var grpNumbers:FlxTypedGroup<AtlasText>;
	...
	override function create() {
		...
		grpNumbers = new FlxTypedGroup<AtlasText>();
		add(grpNumbers);
		...
		for (i in 0...4)
		{
			...
			var numberVal:AtlasText = new AtlasText(note.x + 40, note.y - 100, Std.string(FlxMath.roundDecimal(Note.arrowColors[note.noteData], 0)));
			grpNumbers.add(numberVal);
		}

		for (i in 0...grpTexts.members.length)
			grpTexts.members[i].alpha = 0.6;
		
		for (i in 0...grpNumbers.members.length)
			grpNumbers.members[i].alpha = 0.6;
			
			
		grpTexts.members[curSelected].alpha = 1;		
		grpNumbers.members[curSelected].alpha = 1;
	}
	...
	public function updateColors(change:Float = 0) {
		...
		grpNotes.members[curSelected].colorSwap.update(change);
		...
		grpNumbers.members[curSelected].text = Std.string(FlxMath.roundDecimal(Note.arrowColors[curSelected], 0));
	}
}